...
Zep 的 node/edge listAPI使用 UUID cursor 分页，
本模块封装自动翻页逻辑（含单页retry），对调用方透明地返回完整list。
"""
...
def fetch_all_nodes(
    ...
    """分页获取graphnode，最多返回 max_items 条（默认 2000）。每页request自带retry。"""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """分页获取graph所有边，返回完整list。每页request自带retry。"""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
