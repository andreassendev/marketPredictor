  ...
  <div class="main-view">
    ...
    <header class="app-header">
      <div class="header-left">
        <div class="brand" @click="router.push('/')">MARKETPREDICTOR</div>
      </div>
      ...
      <div class="header-center">
        <div class="view-switcher">
          <button 
            ...
            @click="viewMode = mode"
          >
            {{ { graph: 'Graf', split: 'Delt', workbench: 'Arbeidsflate' }[mode] }}
          </button>
        </div>
      </div>
      ...
      <div class="header-right">
        <div class="workflow-step">
          ...
          <span class="step-name">Simulering</span>
        </div>
...
// Get maxRounds from query params at init
...
const handleGoBack = async () => {
  ...
  addLog('Preparing to go back to Step 2, stopping simulation...')
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        addLog(`Shutdown failed, trying force stop...`)
        try {
      ...
      // Environment not running, checking if process needs stopping
...
const handleNextStep = () => {
  // Step3Simulation 组件会直接处理Rapportgenerering和路由跳转
...
const loadGraph = async (graphId) => {
  // During simulation, auto-refresh skips full loading to avoid flicker
...
const stopGraphRefresh = () => {
  if (graphRefreshTimer) {
    ...
    addLog('Stopping graph live refresh')
  }
}
...
onMounted(() => {
  ...
  // Log maxRounds config (value from query params at init)
  if (maxRounds.value) {
