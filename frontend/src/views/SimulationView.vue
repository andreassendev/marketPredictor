    ...
    <header class="app-header">
      ...
      <div class="header-right">
        <div class="workflow-step">
          ...
          <span class="step-name">Miljøoppsett</span>
        </div>
...
const checkAndStopRunningSimulation = async () => {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      addLog('Simulation environment detected, shutting down...')
      ...
      try {
        ...
        if (closeRes.success) {
          ...
          addLog(`Shutdown failed: ${closeRes.error || 'Unknown error'}`)
          // If graceful shutdown fails, try force stop
          await forceStopSimulation()
        }
      } catch (closeErr) {
        addLog(`Shutdown exception: ${closeErr.message}`)
        // If graceful shutdown exception, try force stop
        await forceStopSimulation()
      }
    } else {
      // Environment not running, checking simulation status
    ...
    // Environment check failure doesn't affect flow
...
onMounted(async () => {
  ...
  // Check and stop running simulation (user returning from Step 3)
