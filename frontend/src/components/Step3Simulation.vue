...
const actionIds = ref(new Set()) // Action IDs for dedup
...
// format化simulation流逝时间（根据rounds次and每roundsminutes数计算）
...
// Reset所有status（用于重新启动simulation）
const resetAllState = () => {
  phase.value = 0
  runStatus.value = {}
  allActions.value = []
...
const doStartSimulation = async () => {
  ...
  try {
    ...
    if (props.maxRounds) {
      ...
      addLog(`设置最大simulationrounds数: ${props.maxRounds}`)
    }
    ...
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ 已清理旧的simulation日志，重新开始simulation')
      }
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 检测simulation是否Fullført（通过 runner_status or平台完成status判断）
